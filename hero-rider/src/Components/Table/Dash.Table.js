...
function DashTable({
    ...
    const handleDeleteAction = async () => {
        await postData(`/admin/users-block`,{userList:checkedUser},{ authorization: authToken });
        return callback();
      };
        ...
        <>
            <div className={tableStyle['table-head-container']}>
                ...
                <div className={`px-5 py-2`}>
                    {/* action button here */}
                    <button onClick={handleDeleteAction} className={`px-3 py-1.5 rounded-md text-white ${!checkedUser.length ? 'opacity-40' : 'opacity-100'} font-medium bg-red-700`} disabled={!checkedUser.length}>Delete</button>
                </div>
            </div>
        </>
    );
